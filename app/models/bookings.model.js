...
Booking.findAllByServiceId = (serviceId, result) => {
    var query = "SELECT properties.name as title, services.description, images.url, locations.city + ' ' + locations.street as location,services.price_per_night as price, bookings.start_date, bookings.end_date FROM bookings INNER JOIN (((services INNER JOIN images on " + serviceId + " = images.service_id AND images.is_main IS NOT NULL)INNER JOIN properties on services.property_id = properties.id)INNER JOIN locations on properties.id = locations.property_id) on bookings.service_id = " + serviceId;
