...
Property.getAllWithDetails = (result) => {
    sql.query("SELECT p.id,p.name,p.user_id,p.phone,p.description,p.rating,price_per_night,city,street,images.url FROM properties p ,services ,locations,images where services.price_per_night = (select MIN(price_per_night) from services where property_id =p.id) AND locations.property_id = p.id AND images.service_id=services.id AND images.is_main =1 ", (err, res) => {
        if (err) {
            console.log(err);
            result(err, null);
            return;
