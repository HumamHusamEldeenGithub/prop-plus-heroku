...
Service.getAllWithDetails = (page_index,result) => {
  sql.query("SELECT *,*,* FROM Services INNER JOIN properties ON properties.id=services.property_id INNER JOIN locations ON services.property_id = locations.property_id LIMIT ? OFFSET ?",[parseInt(process.env.ITEM_PER_PAGE) , process.env.ITEM_PER_PAGE * page_index], (err, res) => {
    if (err) {
      console.log(err);
      result(err, null);
      return;
